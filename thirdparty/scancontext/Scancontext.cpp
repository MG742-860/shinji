...
std::pair<int, float> SCManager::detectLoopClosureIDBetweenSession(
  std::vector<float>& _curr_key,
  ...
  polarcontext_tree_batch_->index->findNeighbors(
    knnsearch_result,
    &curr_key[0] /* query */,
    nanoflann::SearchParams(10));  // error here
  ...
  if (min_dist < SC_DIST_THRES) loop_id = nn_idx;
...
std::pair<int, float> SCManager::detectLoopClosureID(void) {
  ...
  if (min_dist < SC_DIST_THRES) {
    ...
    cout << "[Loop found] Nearest distance: " << min_dist << " btn " << polarcontexts_.size() - 1 << " and " << nn_idx
         << "." << endl;
